...
function somar(consumption, produto, preco) {
  const soma = consumption.reduce((acc, consuptionItem) => {
    if (consuptionItem === produto) {
      const sumAcc = acc + preco;
      return sumAcc;
    }
...
const consumptionSum = (consumption, menu) => {
  let sum = 0;
  for (let category of Object.entries(menu)) {
    for (let item of Object.entries(category[1])) {
      sum += somar(consumption, item[0], item[1]);
    }
  }
  return sum;
};
...
function createMenu(obj) {
  ...
  let menu = obj;
  const objetofetchMenu = {
    fetchMenu: () => menu,
    consumption: [],
    order: (str) => {
      objetofetchMenu.consumption.push(str);
    },
    pay: () => {
      const sum = consumptionSum(objetofetchMenu.consumption, objetofetchMenu.fetchMenu());

      return parseFloat((sum) * 1.1).toFixed(2);
    },
  };

  return objetofetchMenu;
}

const parametroCreatMenu = {
  food: {
    churrasco: 10.5,
    pastel: 2.5,
    coxinha: 3.9,
...
const meuRestaurante = createMenu(parametroCreatMenu);
