...
-- 01. Precisamos identificar os dados do cliente com o e-mail LEONARD.SCHOFIELD@sakilacustomer.org. 
	-- As informações podem ser encontradas na tabela customer
SELECT * FROM customer
	WHERE email = 'LEONARD.SCHOFIELD@sakilacustomer.org';


-- 02. Precisamos de um relatório dos nomes dos clientes, em ordem alfabética, 
...
-- 03. O setor financeiro quer saber título, descrição, ano de lançamento e valor do custo de substituição (replacement_cost), 
...
-- 04. Quantos clientes estão ativos e na loja 1? 
	-- As informações podem ser encontradas na tabela customer
SELECT COUNT(*) AS 'Clientes Ativos' FROM customer
	WHERE store_id = 1
    AND active = TRUE;
    
-- 05. Mostre todos os detalhes dos clientes que não estão ativos na loja 1. 
	-- As informações podem ser encontradas na tabela customer  
SELECT * FROM customer
	WHERE store_id = 1
    AND active = FALSE;
    
-- 06. Precisamos descobrir quais são os 50 filmes feitos apenas para adultos com a menor taxa de aluguel, 
