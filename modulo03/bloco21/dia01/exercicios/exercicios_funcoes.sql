...
-- 7. Escreva uma query que exiba a quantidade de dinheiro necessária 
    ...
    SELECT
		job_id,
        SUM(salary) AS media_salario
        FROM employees
        GROUP BY job_id;
    
-- 8. Utilizando a query anterior, faça as alterações para que seja exibido somente 
	-- a quantidade de dinheiro necessária para cobrir a folha de pagamento 
...
-- 17: Escreva uma query que exiba o sobrenome
	-- e a data de contratação de todos os funcionário contratados em julho de 1987.
SELECT 
	last_name, 
    hire_date
	FROM employees
